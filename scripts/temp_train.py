...
parser.add_argument('--unfreeze-epoch', type=int, default=10, help='Epoch to unfreeze CNN backbone')
...
model = HybridDetector(num_classes=2, pretrained=False, freeze_cnn=args.freeze_cnn).to(DEVICE)
...
optimizer = optim.Adam([
    ...
    {'params': model.cnn.parameters(), 'lr': args.lr / 100}  # Lebih kecil lagi
], lr=args.lr, weight_decay=1e-5, eps=1e-8)
...
log(f"   Auto shutdown: {args.shutdown}")
log(f"   Shutdown delay: {args.shutdown_delay} seconds")
log("")

try:
    for epoch in range(start_epoch, EPOCHS):
        # if args.freeze_cnn and epoch == args.unfreeze_epoch:
        #     print(f"\n🔓 Unfreezing CNN backbone at epoch {epoch+1}")
        ...
        #     ], lr=args.lr)
        #     print(f"   CNN LR set to {args.lr / 100:.2e}")
        # ========== TRAINING PHASE ==========
        model.train()
        train_loss = 0.0
        train_correct = 0
        ...
        for img, dct, label in pbar:
            img, dct, label = img.to(DEVICE), dct.to(DEVICE), label.to(DEVICE)
            optimizer.zero_grad()
            ...
            if USE_AMP:
                scaler.scale(loss).backward()           # 1. Backward with scaled loss
                scaler.unscale_(optimizer)              # 2. Unscale gradients FIRST
                torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=1.0)  # 3. Clip
                scaler.step(optimizer)                  # 4. Update weights
                scaler.update()                         # 5. Update scaler
            else:
                loss.backward()
    ...
    log("\n" + "="*60)
    log("❌ TRAINING FAILED DUE TO ERROR")
    log("="*60)
